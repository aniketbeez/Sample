...
//ct++;
		...
		bnd= cvBoundingRect(contours2, 0);
...
//cvPutText (def,"s1",s1, &font, cvScalar(255,255,255));
//cvPutText (def,"s2",s2, &font, cvScalar(255,255,255));
//cvPutText (def,"s3",s3, &font, cvScalar(255,255,255));
//cvPutText (def,"s4",s4, &font, cvScalar(255,255,255));
//cvPutText (def,"cen",cvPoint(yx,yy), &font, cvScalar(255,255,255));			 
 // if(((s1.x<yx)&&(yx<s2.x))  && ((s1.y<yy)&&(yy<s3.y)))       
...
CvScalar  hsv_min = cvScalar(20, 100, 100, 0);
 ...
 state2=0;
  ...
  buf3 = cvCreateImage( size, frame->depth, 1);
...
cvDilate(hsv_mask1, hsv_mask1 , 0, 7);
   ...
   cvThreshold( hsv_mask1, buf3,40, 1, CV_THRESH_BINARY );
    
cvFindContours(buf3,buf,&cont2,sizeof(CvContour),CV_RETR_LIST,CV_CHAIN_APPROX_SIMPLE);
...
//int ct=0;
...

		{
			...
			result = fabs( cvContourArea( cont2, CV_WHOLE_SEQ ) );

			if( result > result2) {result2 = result; contours2 = cont2;};
